...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.gate2 = torch.nn.Parameter(
            torch.ones(1, self.n_local_heads, 1, 1) * -args.bias
        )
    ...
    def forward(
        ...
        if adapter is not None:
            ...
            adapter_k = (
                self.wk(adapter)
                .view(1, adapter_len, self.n_local_heads, self.head_dim)
                .repeat(bsz, 1, 1, 1)
            )
            adapter_v = (
                self.wv(adapter)
        ...
        if adapter is not None:
            ...
            if video_start:
                ...
                vt_scores[
                    :,
                    :,
                    video_start + self.max_feats :,
                    video_start : video_start + self.max_feats,
                ] = (
                    vt_scores[
                        :,
                        :,
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, args, tokenizer):
        ...
        self.adapter_query = Embedding(
            params.adapter_len * params.adapter_layer, params.dim
        )
        ...
        self.infer_criterion = torch.nn.CrossEntropyLoss(
        ...
        if self.args.edl_mse:
            self.scale_proj = Linear(
                params.vocab_size, 1, bias=True
            )
